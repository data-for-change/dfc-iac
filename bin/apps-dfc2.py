...
def deploy_cleanup_old_logs_cronjob():
    scp('./bin/cleanup_old_logs.py', '~/cleanup_old_logs.py')
    ssh('''
        sudo rm -f /etc/cron.d/cleanup-old-logs
        echo "0 0 * * * root python3 /home/ubuntu/cleanup_old_logs.py /data" | sudo tee /etc/cron.d/cleanup-old-logs
    ''')
